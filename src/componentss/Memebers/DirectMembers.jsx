...
import { getDirectMembers } from '../../Features/Franchise/FranchiseSlice';
...
//   const members = useSelector((state) => state.User.allProfiles);
  ...
  const filteredUsers = members?.filter((user) => {
    ...
    const isActivePlan = searchActivePlan === 'active' ? user?.plans?.find(isPlanActive) : searchActivePlan === 'inactive' ? !user?.plans?.find(isPlanActive) : true;
    const matchesSearch = (
      ...
      // Combine search query match and plan status match
  ...
  return (
    <div className="container mt-5" id='table-margin-top'>
      {/* Search Bar */}
      <div className="mb-3">
        <input
          type="text"
      ...
      <div className="mb-3">
        <label htmlFor="active-plan" className="form-label">Active Plan:</label>
        <select
          id="active-plan"
