...
import { deleteMember, GetUsersWithActivePlans } from '../Features/Admin/AdminSlice';
...
const PaidMembers = () => {
  ...
  const users = useSelector((state) => state.Admin.usersWithActivePlans);
  useEffect(() => {
        ...
        <table className="table table-striped table-bordered table-hover">
          <thead className="table-dark">
            <tr>
              ...
              <th className="text-nowrap">Plan Name</th>
          ...
          <tbody>
            {currentEntries?.map((user) => (
              <tr key={user.user_id}>
                ...
                <td className="text-nowrap">{user?.plans[user?.plans?.length - 1]?.plan?.name}</td>
                <td className="text-nowrap">{formatDate(user?.plans[user?.plans?.length - 1]?.expiryDate)}</td>
