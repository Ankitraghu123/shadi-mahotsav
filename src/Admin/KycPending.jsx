...
const KycPending = () => {
  ...
  const allFranchise = useSelector((state) => state.Franchise?.getAllFranchises?.franchises);
  const {kycApproved} = useSelector(state => state.Franchise)
  const franchises = allFranchise?.filter(
    (franchise) => !franchise.kycId || franchise.kycId?.approved === false
  );
  useEffect(() => {
